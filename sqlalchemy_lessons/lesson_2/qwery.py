...
# .join() -- когда нужно провести фильтрацию \ поиск данных из таблицы А на основе значений
# Пример: взять пользователей в роли "author"

stms = (
    select(User)
    # .join(Role)   # первый вариант
     .join(Role, Role.id == User.role_id) # второй вариант - более безопасный
    # .join(User.role)    # третий вариант - релейшеншипс
...
# Join нужен для присоединения одной таблицы - подгружает данные только в таблицу
# Если после этого нужно ДАННЫЕ ПОДГРУЗИТЬ:
# .joinedload()
# .subqueryload()
# .selectionload()
